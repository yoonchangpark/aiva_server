...
async def call_openai_script(topic: str, context: str):
    ...
    주어진 트렌드 데이터와 팩트 정보를 바탕으로 약 85~95초 분량의 자극적이고 밀도 높은 유튜브 쇼츠 대본을 작성해.
    ...
    2. 숏츠를 위해 한 Scene당 narration 분량은 약 2~3.5초(약 13~22자) 단위로 잘게 쪼개어라(Micro-segmentation). 한 문장이 길면 두 Scene으로 나눠라.
    3. 인접한 Scene에서 `search` 쿼리가 절대 겹치지 않게 무조건 바꿔라.
    4. ★분량 절대 규칙: 대본 전체 길이는 약 85~95초(모든 narration 합산 440~560자)로 서사를 충분히 담아라. 반드시 22~28개 Scene으로 구성하라(절대 12개 이하로 끝내지 마라 — 너무 짧으면 실패다).
