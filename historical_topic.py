...
async def pick_history_topic(exclude_topics: list[str] | None = None,
    ...
    context = (
        ...
        "전체 약 85~95초 / 반드시 22~28개 Scene으로 구성하라 — ★이 범위(22~28개)를 벗어나면(21개 이하 포함) 명백한 실패작이다. "
        "참고 예시 대본처럼 한 호흡씩 잘게 쪼개 충분한 길이를 확보하라. 다 쓴 뒤 Scene 개수를 직접 세어보고 22개 미만이면 장면을 더 쪼개서 채워 넣어라.\n"
